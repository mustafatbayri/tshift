# Altın Senaryolar A1–A12 — Kabul Ölçütleri (v5) · **ONAYLANDI**

**Durum:** ✅ **On iki senaryonun tamamı ve altı varsayımın tamamı onaylandı.**
Fikstür yazımı bu sürümden ilerler.
**Tarih:** 15 Eylül 2026
**Önceki sürümler:** `../v1/` `../v2/` `../v3/` `../v4/` — dondurulmuş.

**v4 → v5 farkı.** Onay turunda gelen ekler işlendi.

| Ne eklendi | Nerede |
|---|---|
| Yönetici plana girdiğinde düşen atamayı **sebebiyle** görür, asistan öneri sunar | A2 madde 3 |
| **Yayın kapısı:** taslak ihlal taşıyabilir, yayınlanmış plan taşıyamaz | A4 madde 7, K-16 |
| Yasal kural ihlali kabul **edilemez**, firma kuralı **edilebilir** | A3 madde 7, K-10 |
| Mola kapsaması **YUMUŞAK** oldu; `MOLA_HAKKI` sert kalıyor | A8 madde 4, K-14 |
| `yasal` bayrağı **üç durumlu**; belirsiz = kabul edilemez | K-17 |
| Altı varsayımın (V-1…V-6) hepsi karara bağlandı | §5 |

> ⚠ **Tek açık kalan:** İki hukuki yorum karara bağlandı ama **uzman gözünden
> geçmedi** — mola eşiğinin brüt süreye uygulanması (K-4) ve `yasal` bayrağı
> sınıflandırması (K-17). Fikstürleri bloke etmiyorlar; sahaya çıkmadan önce
> hukukçu bakmalı.
...
5. [Varsayımlar — hepsi karara bağlandı](#5-varsayımlar--hepsi-karara-bağlandı)
6. [Kararlar K-1…K-17](#6-kararlar-k-1k-17)
7. [Onay durumu](#7-onay-durumu)
...
*Bu hesap programatik doğrulandı ve **Mustafa tarafından onaylandı** (15 Eylül).*
...
3. **(b)** Yöneticiye **bildirim** gider: hangi çalışan, hangi gün, hangi
   plandan düştü. **Bildirim yetmez:** yönetici plana girdiğinde düşen atamayı
   ve **sebebini (izin)** plan ekranında görür, ve **asistan ona öneri sunar** —
   boşluğu kim kapatabilir.
...
7. Yasal dayanaklı kurallar (`yasal = true`) bu yolla kabul **edilemez** —
   sistem kabul seçeneğini **sunmaz bile.** İlk yardım kuralı firma kuralıdır,
   kabul edilebilir. Mustafa'nın ifadesi: *"Yasal kuralların ihlali kabul
   edilemez ama firma kuralı ihlali kabul edilebilir."*
8. Bayrağı **belirsiz** olan kural, yasal gibi davranılır — kabul edilemez
   (K-17 güvenli varsayılanı).
...
**Kaynak:** 1–5, 7 `[karar K-10]` · 6 `[çıkarım]` · 8 `[karar K-17]`
...
7. **Elle müdahale:** Yönetici Ali'nin Çarşamba vardiyasını 09:00'a çekerse
   ...
   karar kullanıcınındır. Kullanıcı devam ederse A3'teki gibi kayda geçer.
8. **Yayın kapısı (K-16).** Cem'in çakışması **düzeltilmeden plan
   yayınlanamaz.** `CAKISMA_YOK` yasal kural olduğu için "kabul ediyorum"
   seçeneği bile sunulmaz. Taslak planda ihlal durabilir — yayında duramaz.
   Mustafa'nın ifadesi: *"Cem'in çakışan mesaisi mantıksız, bu çözülmeden plan
   yayınlanamaz. Zaten yasal ihlal de var."*
...
6 `[karar V-1]` · 7 `[karar K-10, K-11]` · 8 `[karar K-16]`
...
4. **Kötü plan denetlenirse** (altı kişinin altısı da 12:00–13:00'te molada)
   **yumuşak** ihlal yazılır: o saatte sahada 0 kişi var, 3 olmalıydı. Plan
   **geçersiz olmaz**, puanı düşer — `MOLA_KAPSAMASI` yumuşak kuraldır (K-14).
   Ağırlığı: DENGELI 7 · KAPSAMA 9 · CALISAN 6.
...
**Dikkat — iki kural karıştırılmamalı.** Yumuşayan **`MOLA_KAPSAMASI`**
(mola sırasında sahada kimse kalması). **`MOLA_HAKKI`** (kişiye mola verilmesi)
**sert kalıyor** ve yasal: 9 saat sahadaki kişiye 60 dakika ara dinlenme vermek
zorunludur.

Karar tutarlı: mola çıktısı öneri niteliğindeyse (7. madde), mola sırasındaki
kapsamayı sert kısıt yapmak kendi kendisiyle çelişirdi. Sahada kimse molayı
plana göre kullanmıyorsa, plana göre hesaplanan bir eksiği "plan geçersiz"
saymak gerçeği yansıtmaz.

Bunun bedeli: motoru molaları kaydırmaya iten tek şey artık **ağırlık.** Çok
düşük verilirse motor molaları üst üste yığar ve plan müşteriye saçma görünür —
7 / 9 / 6 bu yüzden hedef kapsamanın hemen altında.

**Kaynak:** 1–3, 7 `[karar K-14]` · 2 `[karar, mevzuat]` · 4 `[karar K-14]` ·
5–6 `[spec §6.2]`
...
6. Ayrım ölçütü: geçmiş penceresi firmanın **ilk yayınlanmış planından**
   öncesine düşüyorsa durum "boş"tur, "eksik" değil. *(V-3 onaylandı: firma
   oluşturma tarihi ölçüt değildir — firma üç ay önce açılıp yeni plan yapmaya
   başlamış olabilir.)*
...
**Dikkat.** *"Motor tekrar çalışıp"* ifadesinin **doğrulayıcının** çalışması
olduğu (kontrol), çözücünün yeniden plan üretmesi olmadığı **onaylandı**.
Boşluklar sessizce doldurulmaz; kullanıcı editörde görür ve kendisi doldurur.

**Kaynak:** 1, 6 `[karar K-15]` · 2–5 `[spec §9.7]` · 7–8 `[çıkarım]`

---

## 5. Varsayımlar — hepsi karara bağlandı

| # | Varsayım | Sonuç |
|---|---|---|
| **V-1** | Çakışmada yalnız çakışma ihlali yazılır, ayrıca dinlenme ihlali yazılmaz | ✅ **Onaylandı.** Tek hata iki satır üretirse ihlal sayısı anlamını yitirir |
| **V-2** | Mola tek bloktur, bölünmez | ✅ **Karar oldu (K-14)** — mevzuat |
| **V-3** | "Geçmiş yok" ölçütü = firmanın ilk yayınlanmış planı | ✅ **Onaylandı** |
| **V-4** | Mola hakkı eşiği **brüt** süreye uygulanır | ✅ **Onaylandı (K-4).** Hata yönü tek taraflı: brüt hesap kanunun istediğinden asla az mola vermez. ⚠ Hukuki yorum, uzman gözü beklemede |
| **V-5** | Yasal kural ihlali kabul edilemez, firma kuralı edilebilir | ✅ **Karar oldu (K-10)** |
| **V-6** | Mola kapsaması sert mi yumuşak mı | ✅ **YUMUŞAK (K-14).** `MOLA_HAKKI` sert kalır |
...
| **K-16** | **Yayın kapısı:** taslak ihlal taşıyabilir, yayınlanmış plan taşıyamaz — sıfır yasal ihlal + kalan sert ihlaller açıkça kabul edilmiş | 15 Eyl |
| **K-17** | **`yasal` bayrağı üç durumlu;** belirsiz olan kural yasal gibi davranılır (kabul edilemez). Emin olunan sınıflandırma K-17'de | 15 Eyl |

## 7. Onay durumu

**Tamamlandı (15 Eylül 2026).** On iki senaryonun tamamı ve altı varsayımın
tamamı onaylandı. A5 ertelendi (K-12), altyapısı korunuyor.

Onay turunun kaydı `../ONAY-DURUMU.md`'de tutulmuştu; bu sürüm o dosyadaki
eklerin işlenmiş hâlidir. Tur kapandığı için o dosya arşive alındı.

**Açık kalan tek konu — fikstürleri bloke etmiyor:** iki hukuki yorum karara
bağlandı ama uzman gözünden geçmedi. Mola eşiğinin brüt süreye uygulanması
(K-4) ve `yasal` bayrağı sınıflandırması (K-17). Sahaya çıkmadan önce hukukçu
bakmalı; şu an `[çıkarım]` etiketini koruyorlar.

## 8. Onay sonrası

1. ✅ Cümleler onaylandı ve donduruldu — bu dosya.
2. **Sıradaki iş: `v5/fikstur/A01.json … A12.json`.** ⚠ v2/v3'teki `A04.json`
   **geçersiz**: eski T-10 sahnesine dayanıyor ve geri alınan K-1'e göre
   yazılmıştı. Yalnız biçim örneği olarak duruyor. A5 fikstürü yazılmayacak.
...
5. **Şartname v1.4** — biriken liste artık uzun, küçük bir düzeltme sürümü
   değil:
...
| `TERCIH_KARSILAMA` yeniden değerlendirilmesi (çalışan tercihi yok) | §6.8 | K-13 |
| Hedef kapsama kabul eşiği %95 | §13.4 | K-8 |
| Önceki sekiz madde | T-1…T-8 | v4 §4 |
